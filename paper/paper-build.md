...
### 4.1 Constraint accretion is the agent-era form of bureaucracy—at machine speed

Human institutions accrete procedure over decades; each scandal yields a form, each form outlives its cause, and the accretion is slowed only by the difficulty of getting committees to meet. Agent organizations run the same loop in hours, with one crucial difference: **the agents who add constraints are not the ones who bear the cost of satisfying them.** Each generation of agents hardens the rules against the failures of the last generation, then hands the accumulated bill to the next. No agent observed the total constraint load, because no mechanism measured it. Local rationality produced global paralysis. The census makes the mechanism precise: of 312 identified constraint topics, 290 (93%) were created in exactly one commit and never revisited; only 21 were ever hardened further, and removals are statistically negligible (13 removal-verb governance commits in the entire history, against 312 additions—a ratchet ratio of 24:1 even counting the config-layer event, 312:1 in commit space). Accretion tracked incidents: constraint births cluster within days of the April triage, the June 3 loop failure, and the two provider-failure bursts. We call this *recursive constraint accretion* and suggest it belongs on the list of multi-agent failure modes, alongside collusion and emergent miscoordination: it requires no malice, no misalignment, and no deception—only optimization pressure plus rule-making authority plus the absence of an amortization mechanism.
...
### 4.3 Endpoint reliability bounded everything; peak intelligence mattered little
...
wg could add gates automatically (failed checks auto-generate repair tasks). It could not remove them automatically, and nothing monitored the *aggregate* cost of gates. The one intervention that worked—large-scale deliberate constraint removal—had no first-class representation in the system. This is the design gap, and it is generalizable: **organizations need a bankruptcy mechanism for rules, not just an enforcement mechanism.**
...
- Governance-vocabulary frequency in commit subjects is a proxy, not a measure of constraint load; it can be gamed and it conflates addition with removal. The gate census (§4.1, Figure 2) replaces it, with two caveats: key-phrase clustering over-fragments, so 312 is an upper bound on distinct constraints (164 high-confidence, 148 medium); and commit-subject vocabulary misses silent in-constraint changes—which is itself an argument for the gate-ledger mechanism proposed in §6.
...
**Suggested figures:**
1. Monthly commit velocity with governance-vocabulary overlay (data above)
2. Gate census (sourced: `accretion-curve.csv`): cumulative constraints added vs. removed over time—0→296 active by September, with the April birth peak (95/month) and the May–June accretion freeze during the collapse
3. Model mix over time, August 8–September 16 only (session-attributable window, marked as such): messages and metered cost per model/endpoint, showing the gpt-5.6-sol baseline, the mid-September gpt-6-astra entry, and the zero-metered free tier; January–July marked as the documented-but-not-session-attributable era
4. The provider-backoff-contract sequence: five hardening commits, zero shipped features
5. Task pass-rate vs. cumulative gate count (the "eye of the needle" curve)
