...
### 4.5 The system shredded its own history continuously, by configuration

The study's central gap was not a crash. The task ledger for January–early July was destroyed **continuously, by design**: wg's archive configuration specifies `archive_retention_days = 7`—archived tasks were purged one week after archiving, all spring. The archive was a revolving seven-day window, not a record. Contributing events complete the picture: the graph at the original checkout became unloadable around June 18 (the daemon logging "Failed to load graph" on thousands of consecutive ticks); the coordination graph was recreated from scratch at the July 4 cutover the system's own recovery tag calls **"overengineered"**; disk exhaustion on August 5 triggered the first true preservation reflex—the atomic quarantine of the damaged graph, which is why July 4–August 5 survives in full (253 archived tasks, the 824-task live graph, attempts, completion records, finalization journals, and the ignition week's configuration series); and the working repo was re-cloned from GitHub on August 7, with the current ledger beginning August 8. Git history survived because it lived on a remote; task history lived only in local state with a seven-day fuse.

Three lessons. First, **retention is a governance parameter too**—the rule that shredded the record was as much a rule as any gate, and it is the one rule nobody reviewed. Second, the absence was carved by the dynamic under study: a system optimized for local correctness (keep storage small, archive aggressively) destroyed exactly the record of its own drift. Third, the operator's prompting record contains no mention of backup or archiving until August 10—four days after the fresh clone—when it asks, simply: "What's the archive process for WG?" Nobody audited the shredder because nobody knew it was running. Forensic completeness requires inventorying state—and its deletion rules—at every layer, before the interesting era is over.
