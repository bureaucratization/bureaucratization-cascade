...
![**Figure 1.** Monthly commit velocity (bars) and governance-vocabulary share of commit subjects (line).](figures/fig1-velocity.png){width=100%}
...
![**Figure 2.** The provider-backoff-contract sequence: cumulative specification size after each hardening commit; the governed functionality never shipped in the period shown.](figures/fig2-ratchet.png){width=100%}

### 3.4 Phase III—collapse (May)

By May, monthly commit volume fell to 131 (-89% from April), and dated-artifact production—worktree baseline manifests enumerating date-named files, an independent measurement surface—fell in the same proportion (1,404 artifacts in April to 104 in May, 13:1). Task-level completion rates for the collapse period are unrecoverable: the coordination state was reset on August 8, and we verified from the August 9 recovery snapshot that the reset preceded the capture, so no surviving state directory contains January–July task records (§4.5). Within the observable windows, task throughput and commit throughput move together. The contemporaneous failure reports describe the operating condition plainly:
...
![**Figure 5.** Daily unique task completions, current-state ledger, with the September 13 enforcement-autonomy boundary marked.](figures/fig5-recovery.png){width=100%}

The system now performs well. Which brings us to the finding we did not expect.

![**Figure 3.** Gate census: constraints added and removed per month, with cumulative active constraints.](figures/fig3-accretion.png){width=100%}
...
### 4.6 A contrast case: same tool, bounded campaign, no cascade

A second deployment of wg by the same principal—a genomics grant application (848 tasks, May 19 to June 5, ~10 active days)—ran the same gate configuration with *more* automation (auto-assignment enabled) and did not cascade. It completed ~95% of tasks at a median task latency of 12 minutes and shipped its deliverable on schedule. Two properties make it informative. First, 78% of its tasks (659 of 848) were governance machinery—quality-passes, verification, evaluation, assignment—a stable 75–84% daily share with no upward trend: the intrinsic overhead tax of gated coordination, visible in a healthy campaign, distinct from accretion. Second, 207 tasks were tagged `skip-eval`: the evaluation loop was scheduled and then systematically bypassed mid-campaign—a manual intervention that is, in effect, hand-performed rule bankruptcy, and which we flag as such rather than as evidence of mechanism absence. The comparison supports the definition's claim that the cascade requires sustained operation and incident-driven hardening over time, not merely the machinery: a bounded sprint reaches its goal before the ratchet can bite. The caveat is real—same principal, same tool, not an independent replication—so we present it as a contrast arm, not a confirmation.
